...
const defaultOnComplete = (ctx, task, err) => {
	if (err) {
		throw err;
	}
};
...
class Task extends Subject {
	constructor(listr, task, options) {
		...
		if (task.onComplete && typeof task.onComplete !== 'function') {
			throw new TypeError(`Expected property \`onComplete\` to be of type \`function\`, got \`${typeof task.onComplete}\``);
		}
		...
		this._onComplete = task.onComplete || defaultOnComplete;
	...
	run(context, wrapper) {
		...
		return Promise.resolve()
			...
			.then(() => this._onComplete(context, wrapper),
				e => this._onComplete(context, wrapper, e))
			.then(() => {
				if (this.isPending()) {
